...
const BookbindingOrder: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const orderData = { ...formData, serviceType: 'bookbinding' };
      ...
      if (response.ok) {
        setFormData({ name: '', phoneNo: '', quantity: '', totalAmount: '', advanceAmount: '' });
        setShowModal(true);
      } else {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '600px', margin: '0 auto' }}>
      ...
      <SuccessModal
        ...
        message="Bookbinding order placed successfully!"
      />
    </div>
  );
};
